 ...
 * 去重保证业务幂等。</p>
 *
 * <p><b>去重原理：</b>{@link #insert} 使用 {@code INSERT OR IGNORE} 语法
 * （SQLite 方言，PostgreSQL 对应 {@code ON CONFLICT DO NOTHING}），当 event_id
 * 已存在时插入 0 行（不报错），以此判断是否重复。</p>
 *
...
public interface ProcessedEventMapper {
     ...
     *
     * <p>使用 {@code INSERT OR IGNORE} 语法，当 event_id 主键已存在时不报错
     * 而是返回影响行数 0，调用方据此判断事件是否已处理过。</p>
     *
     * @param eventId     事件 ID
