...
ALTER TABLE hedge_batch_items ADD COLUMN IF NOT EXISTS trace_id VARCHAR(64);
CREATE INDEX IF NOT EXISTS idx_hedge_batch_items_trace ON hedge_batch_items(trace_id);

COMMENT ON COLUMN hedge_batch_items.trace_id IS '分布式链路追踪 ID，入桶时从 MDC 写入（关联原始客户请求），出桶发 hedge-fill-event 时取值赋给 event，保证聚合模式下事件可追溯';
